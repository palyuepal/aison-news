(()=>{
  ...
  const levelLabel=item=>({breaking:'● 重大快訊',update:'◆ 即時更新',followup:'↻ 持續跟進'})[item.level]||(item.active!==false?'● 重大快訊':'◆ 即時更新');
  ...
  ensureTimelineLink();
  if(!items.length){
    root.innerHTML='<div class="live-empty"><span class="live-pulse"></span><div><b>新聞流整理中</b><small>已核實嘅 AI 更新會按香港時間顯示。</small></div></div>';
    return;
  }
