        ...
        "stories": {
            ...
            "items": {
                ...
                "properties": {
                    ...
                    "readTime": {"type": "string", "pattern": "^[5-8] 分鐘$"},
                    "freshness": {"type": "string"},
                    "whatHappened": {"type": "string", "minLength": 150},
                    "reportingContext": {"type": "string", "minLength": 150},
                    "whyImportant": {"type": "string", "minLength": 150},
                    "whatToWatch": {"type": "string", "minLength": 90},
                    "hkImpact": {
                        "type": "array",
                        "minItems": 3,
                        "maxItems": 4,
...
7. 每篇必須寫成可獨立閱讀的詳盡報導：summary 交代核心結論；whatHappened 清楚寫
   已核實事實、時間及範圍；reportingContext 補足背景及與現有產品／市場的關係；
   whyImportant 分析影響；whatToWatch 寫下可以核實的下一個觀察點。不可重複同一句。
...
readTime 只可用「5 分鐘」「6 分鐘」「7 分鐘」「8 分鐘」。
"""
...
def main():
    ...
    for edition_rank, story in enumerate(stories, start=1):
        ...
        row = {
            ...
            "reportingContext": story["reportingContext"].strip(),
